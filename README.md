...
The main difference between a biological process and its artificial counterpart is that, in artificial systems, you have more control over the input values (the independent variables) that affect the process. In biological processes, these inputs are harder to control or predict.
In biological systems, you can't control how strongly a smell affects you, how piercing a screeching sound feels, or how slippery your hands get after repeatedly losing in FIFA 18. These are natural, uncontrollable reactions to sensory inputs.
...
Observations:
It is equally important to note that each variable does not stand alone. They are together as a singular observation. For example, you may list a person’s height, age, and weight. These are three different descriptors, but they describe one individual person.
Now, Once these inputs pass through the main neuron, they are processed and transformed into output values.
...
If he's wandering through the soft, pink terrain of the human brain, he'll eventually come across the canyon we mentioned earlier, and he'll need to cross it.

Maybe there's a party waiting on the other side, with music, drinks, and fun. In the brain, he has to make the leap. But in a Neural Network, it's much easier—he doesn't even need a bike to reach his goal.
...
The weights decide how much of a signal gets passed through the network. During training, you adjust these weights to improve the network’s performance, similar to how lifting weights strengthens your body.
...
The last section and this one are closely connected because the activation function is the process used on the weighted input value when it enters the neuron.
Values:
...
Here’s the second method: the Sigmoid function. It turns values below 0 into 0 and values above 0 into 1, but with a smooth curve instead of a hard cut-off. This curve makes it much better for handling probabilities, especially at the output layer of a Neural Network. If a threshold function tells you only if the value is 0 or 1, the Sigmoid gives you all the details in between, like every cent between 0 and 1 dollar.
...
Well, it’s time-consuming, boring, and every ill-considered choice can take you a step closer to long-term financial bondage. It’s so stressful. Every option becomes an imperative you must painstakingly consider, and objectively at that. It's almost a puzzle to solve.
...
We begin with the four variables on the left and the top neuron of the hidden layer in the middle. All four variables all are connected to the neuron by synapses.
...
The Cost Function
Then we apply what is called a cost function (Cost function - Wikipedia), which is one half of the squared difference between the output value and actual value. This is just one commonly used cost function. There are many. We will apply this particular one in our calculations.
...
If you make the wrong alteration it would be like having a car with the front axle pointing slightly to the left, and while you are driving you let go of the steering wheel but keep your foot on the gas.


Sooner or later, you’re going to crash.

Two ways to adjust the weights
The first is the brute-force approach. This is far better suited to a single-layer feed-forward network. Here you take a number of possible weights. On a graph it looks like this:
...
You could have an almost infinite amount of weights to adjust. Therefore a process of elimination would be like looking for the prettiest grain of sand on a beach. Good luck with that. This problem is known as the Curse of Dimensionality.
...
Just to remind you, the weights are the only thing we can adjust as we look to bring the difference between our output value and our actual value as close to 0 as possible. This is important because the smaller the difference between those two values, the better our Neural Network is performing.


Gradient Descent
You can see the aforementioned method illustrated in the graph below.

Gradient Descent is a time-saving method with which you are able to leapfrog large chunks of unusable weights by leaping down and across the U-shape on the graph. This is great if the weights you have amassed fit cleanly into this U-shape you see above. This happens when you have one global minimum, the single lowest point of all your weights. However, depending on the cost function you use, your graph will not always plot out so tidily.
